...
function onResultsPose(results) {
    ...
    if (areArmsCrossed(results.poseLandmarks)) {
        // Provide feedback for crossed arms
        console.log("Arms are crossed!");
        // we can also add other feedback mechanisms here, such as visual cues on the canvas
    }
...
function areArmsCrossed(poseLandmarks) {
    // Extract landmarks for wrists, elbows, and shoulders
    let leftShoulder = poseLandmarks[11];
    let rightShoulder = poseLandmarks[12];
    let leftElbow = poseLandmarks[13];
    let rightElbow = poseLandmarks[14];
    let leftWrist = poseLandmarks[15];
    let rightWrist = poseLandmarks[16];

    // Calculate distances between wrists and opposite elbows/upper arms
    let distanceLeftWristToRightElbow = calculateDistance(leftWrist, rightElbow);
    let distanceRightWristToLeftElbow = calculateDistance(rightWrist, leftElbow);
    let distanceBetweenWrists = calculateDistance(leftWrist, rightWrist);

    // Define a threshold for determining if arms are crossed
    let threshold = 0.1;  // This is an initial guess. Adjust based on testing.

    // Check if arms are crossed based on the distances calculated
    if (distanceLeftWristToRightElbow < threshold && distanceRightWristToLeftElbow < threshold && distanceBetweenWrists < threshold) {
        return true;
    }
