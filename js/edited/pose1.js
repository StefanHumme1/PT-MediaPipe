...
function cachePoseLandmarks(results) {
    // Using recording start time as the timestamp
...
function addLandmarkNamesToData(data) {
    for (let entry of data) {
        for (let idx = 0; idx < entry.length; idx++) {
            let landmarkName = POSE_LANDMARKS[idx] || "unknown";
            entry[idx].landmark.name = landmarkName;
        }
    }
    return data;
}
...
function saveCachedLandmarks() {
    ...
    const landmarksDataWithNames = addLandmarkNamesToData(landmarksCache);
    const exportData = {
